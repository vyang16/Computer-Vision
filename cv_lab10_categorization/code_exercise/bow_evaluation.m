...
sizeCodebook = 200;
%vCenters = create_codebook('../data/narcissus-training-pos','../data/cars-training-neg',sizeCodebook);
%vCenters = create_codebook('../data/narcissus-training-pos','../data/orleander-training',sizeCodebook);
...
%vBoWPos = create_bow_histograms('../data/narcissus-training-pos',vCenters);
vBoWPos = create_bow_histograms('../data/cars-training-pos',vCenters);
disp('processing negative training images');
%vBoWNeg = create_bow_histograms('../data/orleander-training',vCenters);
...
%vBoWPos_test = create_bow_histograms('../data/narcissus-testing-pos',vCenters);
vBoWPos_test = create_bow_histograms('../data/cars-testing-pos',vCenters);
disp('processing negative testing images');
%vBoWNeg_test = create_bow_histograms('../data/orleander-testing',vCenters);
...
prob_nearest = [prob_nearest; bow_recognition_multi(test_histograms, labels, vBoWPos, vBoWNeg, @bow_recognition_nearest)];
disp('______________________________________')
disp('Bayesian classifier')
prob_bayes = [prob_bayes; bow_recognition_multi(test_histograms, labels, vBoWPos, vBoWNeg, @bow_recognition_bayes)];
disp('______________________________________')
