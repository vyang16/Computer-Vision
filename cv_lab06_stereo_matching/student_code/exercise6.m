...
%run('C:\Users\Viviane Yang\polybox2\Master\Herbstsemester 2019\Computer Vision\exercise\cv_lab06_stereo_matching\code\GCMex-master\GCMex-master\compile_gc.m')
%addpath('GCMex-master\GCMex-master\')
...
%close all;
% to get a good guess
% IDEA, match points using last weeks assignment, get range that is the max
% and the min from those matched points difference use RANSAC

%maxR = getDispRange(imgRectL, imgRectR, 40);
% Leave this exercise for the end, and for now try these fixed ranges
%dispRange = -maxR:maxR;
%maxR
dispRange = -8:8; %works well
%%
% Compute disparities, winner-takes-all
% (exercise 5.1)
dispStereoL = ...
   stereoDisparity(rgb2gray(imgRectL), rgb2gray(imgRectR), dispRange, "SSD");
dispStereoR = ...
   stereoDisparity(rgb2gray(imgRectR), rgb2gray(imgRectL), dispRange, "SSD");
