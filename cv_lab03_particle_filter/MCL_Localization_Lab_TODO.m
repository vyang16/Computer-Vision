...
measurement = door1 + door2 + door3; % dim(1,simpar.domain)
end
...
% TODO: Change this value for the correct one
N = size(priorbelief_particles,2)
predict_particles = priorbelief_particles + uk; 
...
p_door = door1 + door2 + door3;
if(zk==1)
    %particles around the doors are more probable
    particle_weights = p_door;
    
else
    %inverse of that
    particle_weights = 1 - p_door;
end
...
%depending on the weight, the probability of the particle to be in
%update_belief_particles is higher or lower
